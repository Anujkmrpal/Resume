...
var _require = require('./dom/impl');

var idlUtils = _require.idlUtils;
...
function setupWindow(window, args) {
  ...
  window._evaluate = function (code, filename) {
    ...
    try {
      ...
      if (typeof code === 'string' || code instanceof String) result = VM.runInContext(code, context, { filename: filename });else if (code) result = code.call(window);
      browser.emit('evaluated', code, result, filename);
      return result;
    } catch (error) {
      if (error && typeof error === 'object') error.filename = error.filename || filename;
      throw error;
    } finally {
  ...
  window._submit = function (formArgs) {
    var url = URL.resolve(document.URL, formArgs.url);
...
function createDocument(args) {
  ...
  var documentImpl = idlUtils.implForWrapper(document);
...
function buildRequest(args) {
  ...
  var url = URL.resolve(site, URL.format(args.url));
